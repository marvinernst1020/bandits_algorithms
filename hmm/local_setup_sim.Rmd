...
Compiling the base models using a string:
```{r}
dummy_data <- list(y = c(0,1), N = 2)

base_models <- lapply(1:K, function(k) {
  jags.model(textConnection(model_string), data = dummy_data, n.chains = 1, quiet = TRUE)
})
```
...
thompson_poor <- function(K,N,theta,y_true,z_true,Batch_size){
  ...
  for (t in 1:N) {
    ...
    if (length(observed_rewards[[a_t]]) >= 2 && t%%Batch_size == 0) { # we only do inference is the arm has been pulled on last iteration
      ...
      # CAHNGE 4
      post <- coda.samples(model, c("theta0", "theta1", "pi",paste0("z[", length(observed_rewards[[a_t]]), "]")), n.iter = 25) # was before 100
