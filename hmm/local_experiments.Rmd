...
### Cumulative Reward:
```{r}
plotts<-ggplot(ts_compare_df, aes(x = time, y = cumulative_reward, color = model)) +
  geom_line() +
  labs(title = "Cumulative Reward: Baseline vs poor TS",
       x = "Time", y = "Cumulative Reward") +
  theme_minimal()

ggsave(file.path(output_dir, "poor_ts_cumulative_reward_single.png"), plot = plotts, width = 8, height = 6, dpi = 300)
```

### Plot Regret:
```{r}
plotts<-ggplot(ts_compare_df, aes(x = time, y = cumulative_regret, color = model)) +
  geom_line() +
...
### Cumulative Regret:
```{r}
plotucb<-ggplot(ucb_compare_df, aes(x = time, y = cumulative_regret, color = model)) +
  geom_line() +
...
```{r}
plt<- ggplot(ucb_ar_compare_df, aes(x = time, y = cumulative_regret, color = model)) +
  geom_line() +
...
## 3.1 Generate Datasets (for 25 runs)
```{r}
...
```{r}
ts_runs_df <- future_lapply(1:1, function(i) {
  bind_rows(
...
### 3.2.2 UCB
```{r}
ucb_runs_df <- future_lapply(1:1, function(i) {
  bind_rows(
    simulate_model_on_run(
      run_id = i, N = N, K = K,
      algorithm = "ucb",
