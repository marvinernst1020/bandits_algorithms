...
system.time({
results<- thompson_advanced (K,N,theta,y_true,z_true,Batch_size=2)
})
...
for (t in 2:N) {
  ...
  if (t <= length(posterior_matrices) && is.matrix(posterior_matrices[[t]])) {
    pi0 <- posterior_matrices[[t]][,"pi[1]"]
    pi1 <- posterior_matrices[[t]][,"pi[2]"]
    z_last <- posterior_matrices[[t]][, paste0("z[", last_z_index, "]")]
    sampled_pi <- ifelse(z_last == 0, pi0, pi1)
    z_t <- rbinom(length(sampled_pi), size = 1, prob = sampled_pi)
  }
  ...
  for (i in 1:K) {
    if (t <= length(posterior_matrices) && is.matrix(posterior_matrices[[t]])) {
      ...
      theta0 <- posterior_matrices[[t]][, paste0("theta[", i, ",1]")]
      theta1 <- posterior_matrices[[t]][, paste0("theta[", i, ",2]")]
...
for (t in 1:N) {
  ...
  if (length(observed_rewards[[a_t]]) >= 2 && t%%10 == 0) { # we only do inference is the arm has been pulled on last iteration
