...
Setting a seed:
```{r}
set.seed(123) # maybe it was see 12
```
...
```{r}
generate_global_datasets(K = 2, N = 5000, mu = mu, pi_global = pi_global, scenario_name = "A", n_runs = 30)
```
...
ts_runs_df <- future_lapply(seq_len(n_runs), function(i) {
  bind_rows(
    simulate_model_on_run(
      ...
      simulate_model_on_run(
      run_id = i, N = 5000, K = 2, batch_size = 100, burn = 1000, n_iter = 200,
      algorithm = "ts",
      complexity = "poor",
      dynamics = "common",
      data_path = "data_global/A",
    ...
    simulate_model_on_run(
      run_id = i, N = 5000, K = 2,
      algorithm = "ts",
      complexity = "advanced_batching", batch_size = 100, burn = 1000, n_iter = 200,
      dynamics = "common",
      data_path = "data_global/A",
      setting = "global"
    )
