...
ggplot(ts_summary, aes(x = time, y = avg_regret, color = model_id)) +
  geom_line(size = 1) +
  labs(title = "Thompson Sampling: Average Cumulative Regret",
...
Run simulations in parallel:
```{r}
ucb_runs_df <- future_lapply(1:25, function(i) {
  bind_rows(
    simulate_model_on_run(
      run_id = i, N = 5000, K = 2,
      algorithm = "ucb",
...
ggplot(ucb_summary, aes(x = time, y = avg_regret, color = model_id)) +
  geom_line(size = 1) +
  labs(title = "UCB: Average Cumulative Regret",
       x = "Time", y = "Cumulative Regret") +
  theme_minimal()
```
