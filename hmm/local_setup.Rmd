...
K        <- 2      # arms
T_max    <- 200   # total pulls
burn_in  <- 700   # JAGS burn-in (im gonna change this to 1000 but its too time expensive now)
n_samps  <-  100   # posterior draws per arm per round

#true parameters for simulating rewards
theta_true <- c(0.3, 0.9)
pi_true    <- c(0.10, 0.95)

#simulate underlying z & y for each arm up to T_max
...
for(t in 1:T_max){
  ...
  for(i in 1:K){
    ...
    if(n_i == 0) {
      ...
      data_jags <- list(y = hist_y[[i]], N = n_i)
      jm <- jags.model(file     = "poor_model.jags",
                       data     = data_jags,
                       n.chains = 2, quiet=TRUE)
      update(jm, burn_in)

      # b) sample theta0, theta1 and last z[n_i]
      vars <- c("theta0","theta1", paste0("z[", n_i, "]"))
      post <- coda.samples(model = jm,
                           variable.names = vars,
                           n.iter = n_samps)
      M    <- as.matrix(post)
      th0  <- M[,"theta0"]
      ...
      # c) compound‐theta 
      th_comp <- ifelse(zN == 1, th1, th0)

      # d) compute UCB = mean + (sd/sqrt(n_i))*ln(t)
      ucb_index[i] <- mean(th_comp) +
                      ( sd(th_comp) / sqrt(n_i) ) * log(t)
    }
  }

  # pick & pull the best arm
  arm_t <- which.max(ucb_index)
  chosen_arm[t] <- arm_t

  # “observe” its reward at time t
  r_t <- sim_y[arm_t, t]
  rewards[t] <- r_t
...
for(i in 1:K){
  cat("Arm", i, "- pulls:", length(hist_y[[i]]),
      "mean reward:", round(mean(hist_y[[i]]),3), "\n")
}
```
