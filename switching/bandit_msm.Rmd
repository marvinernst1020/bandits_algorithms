...
# 1. Generating Simulated State Transitions

We will start generating state transitions that will guide the probabilities of each arm of the bandit problem. We initiate using a stable setup where the probability of transition is low (10 %) and the transitions between states are not very frequent (We directly define a transition matrix).
```{r generate-simulated-hmm-data}
init_prob <- c(0.5,0.5) # w set initial probability of each state as equal
...
# 2. Bandit Algorithms (UCB and Thompson Sampling)
...
As latent state rarely switches, the environment is almost stationary most of the time. UCB thrives in stable settings as the optimal arm will remain the same for long periods of time and UCB is quick to figure out the better arm early on while Thompson might experience more exploration.


# 3. Computing Average Performance
...
Compared to the previous plot, this plot is statistically more reliable than a single run. It smooths over noise and gives a reliable average performance picture with variability bands. We can clearly observe that UCB perfoms better in settings with higher stickiness.

# 4. Lower Transition Stickiness
...
# 5. Implementing Sliding Window UCB
...
# 10c. Discounted UCB with State-Aligned Optimal Reward
...
# 13. Time-Varying Within Arm-Rewards

In the section 13 and 15 we will be experimenting with Time-Varying Arm Rewards. This section deals with the case of Discrete Switching while section 15 deals with a Smoother Variation of Switching via Autoregressive processes (also mentioned in James Doulgas' book as Hamilton Markov‑Switching Autoregressive, MS-AR). While a Discrete Switching method is much simpler than Hamilton's MS-AR, it is still time varying.

The purpose of the code in this section is to	simulate a 2-state Markov chain and use the hidden state to determine arm reward probabilities. Each arm has different values for each state. 


```{r}
# 13. Time-Varying Within Arm-Rewards (Discrete Markov Switching) 

set.seed(2050)
n_bandit <- 500
n_runs   <- 200

trans_mat <- matrix(c(0.9, 0.1, 0.1, 0.9), 2, 2)
...
simulate_state_dependent_arms <- function(n = 500, 
                                          trans_mat = matrix(c(0.9, 0.1, 0.1, 0.9), 2, 2),
                                          arm1_probs = c(0.8, 0.3),
                                          arm2_probs = c(0.4, 0.7)) {
  states <- numeric(n)
  ...
  # arm reward probabilities based on state
  for (t in 1:n) {
    arm1[t] <- arm1_probs[states[t]]
    arm2[t] <- arm2_probs[states[t]]
  }
  
  list(p1 = arm1, p2 = arm2, state = states)
}

sim <- simulate_state_dependent_arms()
arm1 <- sim$p1
arm2 <- sim$p2
state <- sim$state
best <- ifelse(arm1 > arm2, 1, 2)
...
Explaining Behavior: The true best arm depends on the latent state — but our algorithms can't observe the state.

- UCB is over committing to arm 1 because in state 1, arm 1 has 0.8 (much better than 0.4).
- Because the environment starts in state 1 and is very sticky (0.9), the early bias toward arm 1 doesn’t get corrected quickly.
- In many runs, the state never even switches long enough for TS or SW to confidently see that arm 2 is better in state 2.

So we see that UCB’s rigidity accidentally is working in its favor. TS and SW are reacting to switching and noise, but looking worse on average because their adaptability doesn’t help enough due to the stickiness.
...
The results that we obtained in this simulation are highly susceptible to changes in switching probabilities. If we were to make switching more frequent by creating more back and forth switching we expect TS and SW to outperform UBC as we expect UBC to not perform well with more frequent switching.
...
ggplot(df_acc_long, aes(t, value, color=variable)) +
  ...
  theme_minimal()
```
We can see that this graph confirms our expectation of UBC performing worse than the other methods.

(we could do cross validation here to test with which params the methods perform best to further discover relationships)


# 3 Arm Case: Global Switching Model
...
bandit_probs <- matrix(c(0.8, 0.4, 0.6,
                         0.3, 0.7, 0.5), nrow = 2, byrow = TRUE)

reg_ucb_global <- matrix(0, nrow = n_bandit, ncol = n_runs)
reg_ts_global <- matrix(0, nrow = n_bandit, ncol = n_runs)
ucb_choices_global <- matrix(0, nrow = n_bandit, ncol = n_runs)
ts_choices_global <- matrix(0, nrow = n_bandit, ncol = n_runs)
best_arm_global <- matrix(0, nrow = n_bandit, ncol = n_runs)
...
for (r in 1:n_runs) {
  ...
  probs <- bandit_probs[states, ]
  best_arm <- apply(probs, 1, which.max)
  best_prob <- apply(probs, 1, max)
  
  succ_ucb <- tr_ucb <- numeric(3)
  alpha_ts <- beta_ts <- rep(1, 3)
  
  cum_ucb <- cum_ts <- 0
  regs_ucb <- regs_ts <- numeric(n_bandit)
  
  for (t in 1:n_bandit) {
    # UCB
    ucbv <- ifelse(tr_ucb == 0, 1, succ_ucb / tr_ucb + sqrt(2 * log(t) / tr_ucb))
    a_ucb <- which.max(ucbv)
    ucb_choices_global[t, r] <- a_ucb
    r_ucb <- rbinom(1, 1, probs[t, a_ucb])
    succ_ucb[a_ucb] <- succ_ucb[a_ucb] + r_ucb
    tr_ucb[a_ucb] <- tr_ucb[a_ucb] + 1
    cum_ucb <- cum_ucb + (best_prob[t] - probs[t, a_ucb])
    regs_ucb[t] <- cum_ucb

    # Thompson Sampling
    ts_draws <- rbeta(3, alpha_ts, beta_ts)
    a_ts <- which.max(ts_draws)
    ts_choices_global[t, r] <- a_ts
    r_ts <- rbinom(1, 1, probs[t, a_ts])
    alpha_ts[a_ts] <- alpha_ts[a_ts] + r_ts
    beta_ts[a_ts] <- beta_ts[a_ts] + (1 - r_ts)
    cum_ts <- cum_ts + (best_prob[t] - probs[t, a_ts])
    regs_ts[t] <- cum_ts
  }
  
  reg_ucb_global[, r] <- regs_ucb
  reg_ts_global[, r] <- regs_ts
  best_arm_global[, r] <- best_arm
}

# Compute cumulative regret
df_global <- tibble(
  t = 1:n_bandit,
  UCB = rowMeans(reg_ucb_global),
  Thompson = rowMeans(reg_ts_global)
) %>% pivot_longer(-t, names_to = "Method", values_to = "CumulativeRegret")

ggplot(df_global, aes(t, CumulativeRegret, color = Method)) +
  geom_line(size = 1) +
  labs(title = "Cumulative Regret: Global Switching Environment (3 Arms)",
       x = "Time Step", y = "Cumulative Regret") +
  theme_minimal()

# Accuracy calculation
compute_accuracy <- function(choices, best_arms) {
  acc <- numeric(n_bandit)
  for (t in 1:n_bandit) {
    acc[t] <- mean(choices[t, ] == best_arms[t, ])
  }
  return(acc)
}

acc_df_global <- tibble(
  t = 1:n_bandit,
  UCB = compute_accuracy(ucb_choices_global, best_arm_global),
  Thompson = compute_accuracy(ts_choices_global, best_arm_global)
) %>% pivot_longer(-t, names_to = "Method", values_to = "Accuracy")

ggplot(acc_df_global, aes(t, Accuracy, color = Method)) +
  geom_line(size = 1) +
  labs(title = "Accuracy: Global Switching (Best Arm Match)",
       x = "Time Step", y = "Fraction Correct") +
  theme_minimal()
```

# 3 Arm Local Switching Model
...
# Transition matrices for each arm's local state
trans_mat_list <- list(
  matrix(c(0.9, 0.1,
           0.1, 0.9), nrow = 2, byrow = TRUE),
  matrix(c(0.85, 0.15,
...
# reward probabilities for each arm in each of its states
# each row corresponds to an arm; columns are states 1 and 2
arm_probs <- list(
  c(0.8, 0.3),
  c(0.4, 0.7),
  c(0.6, 0.5)
...
reg_ucb_local <- matrix(0, nrow = n_bandit, ncol = n_runs)
reg_ts_local <- matrix(0, nrow = n_bandit, ncol = n_runs)

for (r in 1:n_runs) {
  # Simulate local state sequences for each arm
  states_list <- list()
  for (a in 1:3) {
    states <- numeric(n_bandit)
    states[1] <- sample(1:2, 1)
    for (t in 2:n_bandit) {
      states[t] <- sample(1:2, 1, prob = trans_mat_list[[a]][states[t - 1], ])
    }
    states_list[[a]] <- states
  }
  
  #reward probabilities for each arm at each time step
  probs <- matrix(0, nrow = n_bandit, ncol = 3)
  for (a in 1:3) {
    probs[, a] <- arm_probs[[a]][states_list[[a]]]
  }
  
  #  best arm and its probability at each time step
  best_arm <- apply(probs, 1, which.max)
  best_prob <- apply(probs, 1, max)
  succ_ucb <- rep(0, 3)
  tr_ucb <- rep(0, 3)
  ...
  cum_ucb <- 0
  cum_ts <- 0
  regs_ucb <- numeric(n_bandit)
  regs_ts <- numeric(n_bandit)
  for (t in 1:n_bandit) {
    # UCB selection
    ucb_values <- ifelse(tr_ucb == 0, 1, succ_ucb / tr_ucb + sqrt(2 * log(t) / tr_ucb))
    a_ucb <- which.max(ucb_values)
    r_ucb <- rbinom(1, 1, prob = probs[t, a_ucb])
    succ_ucb[a_ucb] <- succ_ucb[a_ucb] + r_ucb
    tr_ucb[a_ucb] <- tr_ucb[a_ucb] + 1
    cum_ucb <- cum_ucb + (best_prob[t] - probs[t, a_ucb])
    regs_ucb[t] <- cum_ucb
    ...
    ts_draws <- rbeta(3, alpha_ts, beta_ts)
    a_ts <- which.max(ts_draws)
    r_ts <- rbinom(1, 1, prob = probs[t, a_ts])
    alpha_ts[a_ts] <- alpha_ts[a_ts] + r_ts
    beta_ts[a_ts] <- beta_ts[a_ts] + (1 - r_ts)
    cum_ts <- cum_ts + (best_prob[t] - probs[t, a_ts])
    regs_ts[t] <- cum_ts
  ...
  reg_ucb_local[, r] <- regs_ucb
  reg_ts_local[, r] <- regs_ts
}
```
...
df_local <- tibble(
  t = 1:n_bandit,
  UCB = rowMeans(reg_ucb_local),
  Thompson = rowMeans(reg_ts_local)
) %>% pivot_longer(-t, names_to = "Method", values_to = "CumulativeRegret")

ggplot(df_local, aes(t, CumulativeRegret, color = Method)) +
  geom_line(size = 1) +
  labs(
    title = "Cumulative Regret: Local Switching Environment (3 Arms)",
    x = "Time Step", y = "Cumulative Regret"
  ) +
  theme_minimal()
```
...
ucb_choices_local <- matrix(0, nrow = n_bandit, ncol = n_runs)
ts_choices_local  <- matrix(0, nrow = n_bandit, ncol = n_runs)
best_arm_local    <- matrix(0, nrow = n_bandit, ncol = n_runs)

# RE-RUN the whole loop, but now also storing choices and best arm
for (r in 1:n_runs) {
  states_list <- list()
  for (a in 1:3) {
    states <- numeric(n_bandit)
  ...
  for (t in 1:n_bandit) {
    ...
    ts_draws <- rbeta(3, alpha_ts, beta_ts)
    a_ts <- which.max(ts_draws)
    ts_choices_local[t, r] <- a_ts
    r_ts <- rbinom(1, 1, prob = probs[t, a_ts])
    alpha_ts[a_ts] <- alpha_ts[a_ts] + r_ts
    beta_ts[a_ts] <- beta_ts[a_ts] + (1 - r_ts)

    best_arm_local[t, r] <- best_arm[t]
  }
}
...
acc_df_local <- tibble(
  t = 1:n_bandit,
  UCB = compute_accuracy(ucb_choices_local, best_arm_local),
  Thompson = compute_accuracy(ts_choices_local, best_arm_local)
) %>%
  pivot_longer(-t, names_to = "Method", values_to = "Accuracy")

# Plot
ggplot(acc_df_local, aes(t, Accuracy, color = Method)) +
  geom_line(size = 1) +
  labs(title = "Accuracy: Local Switching (Best Arm Match)",
       x = "Time Step", y = "Fraction Correct") +
  theme_minimal()
...
ggplot(acc_df, aes(t, accuracy, colour = Method)) +
  ...
  theme_minimal()
```
All three lines spike early because every algorithm initially treats all five arms equally, so lucky guesses momentarily lift accuracy. UCB then dives and slowly climbs back—it over-commits to the arm that looked best in the first state and must re-explore after the state flips. Sliding-Window UCB (30) ends up at the top (0.40) because discarding old data lets it track the current state fastest. Thompson Sampling settles in between bcs of its Bayesian updates which need more evidence than SW-UCB.There is an accuracy ceiling of about 40% while chance is 20%.

# 15. Non-Stationary Rewards 

Here I'm doing Non‑Stationary Rewards driven by James Hamilton’s Markov‑Switching Autoregressive thingie in the book. Now we are  simulating a true non-stationary reward environment using Markov-Switching Autoregressive (MS-AR) processes — this is much more realistic and dynamic than standard discrete switching.
The mean reward evolves over time using autoregressive dynamics:
z_t = \mu_{s_t} + \phi(z_{t-1} - \mu_{s_t}) + \sigma \cdot \epsilon_t
...
We can clearly observe that UCB performs the worst as it is vulnerable to new changes and tends to stick with long-term averages weigh old data heavily. UCB Might keep favoring an arm that was best early on but no longer is. This it why UCB can be performing quite well in the begging periods but starts to perform in a worse manner in later stages.
