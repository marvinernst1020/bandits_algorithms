...
simple_df <- do.call(rbind, lapply(names(results), function(name) {
  regrets <- sapply(results[[name]], function(r) r$last_simple_regret)
  data.frame(SimpleRegret = regrets, Algorithm = name)
})) %>%
  filter(!is.na(SimpleRegret))

ggplot(simple_df, aes(x = SimpleRegret, fill = Algorithm, color = Algorithm)) +
  ...
  scale_color_manual(values = c("Standard UCB" = "#D62728", 
                                "Thompson Sampling" = "#1F77B4", 
                                "Tuned UCB" = "#2CA02C")) +
  labs(x = "Final Simple Regret", y = "Density") +
  theme_classic(base_size = 12) +
  theme(legend.position = "bottom", legend.title = element_blank())
```
